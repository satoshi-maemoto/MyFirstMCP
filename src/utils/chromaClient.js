...
async function getOrCreateCollection() {
  try {
    // 既存コレクション一覧取得
    const listRes = await axios.get(`${CHROMA_URL}/api/v1/collections`);
    console.log('Chroma API response:', JSON.stringify(listRes.data, null, 2));
    ...
    if (found) {
      console.log(`Chroma v1: collection '${COLLECTION_NAME}' found.`);
      return found;
    }
    // なければ新規作成
    ...
    console.log(`Chroma v1: collection '${COLLECTION_NAME}' created.`);
